...
<div class="container margentop5" style='text-align:center'>
                    <?php 
    ...
    $_POST['url']=$url;
                    ...
                    $result = $conn->query("SELECT id_tema,fecha,titulo,texto,usuario_creador from temas where id_escritor=$id_escritor");
if ($result->num_rows > 0) {
    while ($fila = $result->fetch_assoc()) {
        ...
        echo "<div class='row'><div class='col'><a href='https://proyectodaw.free.nf/tema.php"."?id_escritor=$id_escritor&id_tema={$fila['id_tema']}'".">Visitar tema</a></div><div class='col'>{$fila['fecha']} XXX MENSAJES</div>";
        echo "</div></div></div>";
    }
} else {
    echo "<p>No hay resultados encontrados.</p>";
}
...
cajaTema.innerHTML+="<input type='hidden' name='usuario' value='<?php echo "{$_SESSION['nombre']}";?>';>";
...
principal.appendChild(cajaTema);
elemento.remove();
