...
$stmt = $conn->prepare("SELECT id_libro,precio, imagen, sinopsis, nombre, url_amazon,numero_paginas,ISBN,editorial FROM libro WHERE id_escritor = ?");
...
if ($result && $result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $precio_formateado = number_format($row['precio'] / 100, 2, ',', '.') . "€";
        $contenido_array[] = [
