...
$id_escritor=null;
$result = $conn->query("Select id_usuario,correo from usuario where nombre = '$nombreUsuario'");
if ($result->num_rows > 0) {
    if ($fila = $result->fetch_assoc()) {
$id_escritor=$fila['id_usuario'];
$correo_escritor=$fila['correo'];
    }
}
$conn->close();
if(!isset($_GET['get_pagina']) && !(basename($path)=='clubdelectura')) {$_GET['get_pagina']=1;}
function indice(){
    global $id_escritor;
...
$result = $conn->query("SELECT titulo from pagina where id_escritor=$id_escritor ORDER BY id_pagina");
if ($result) {
    if ($result->num_rows > 0) {
        $contador=1;
...
?>
<header>
    <div class="logoDiv centrar" style='width:12.5%'><a href='https://proyectodaw.free.nf'><img src='../logo-proyecto-daw.jpg' style='width:100%;cursor:pointer;' title='púlsame para ir al menú principal'></a></div>
    <div class="row m-0 centrar" style='width:87.5%'>
        <?php indice();?>
        <script>
        if(<?php echo json_encode(basename($path)=='clubdelectura');?>){
          pagina=4;
        } else {
            pagina=<?php echo json_encode($_GET['get_pagina'] ?? '');?>;}
        let paraSubrayar = document.getElementById('marcar_'+pagina);
        console.log(pagina+"AA");
